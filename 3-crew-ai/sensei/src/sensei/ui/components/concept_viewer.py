...
def render_concept(title: str, content: str) -> None:
    """Render concept content with markdown and LaTeX support.
    ...
    syntax highlighting and LaTeX math formulas.
    
    LaTeX formulas using \\(...\\) and \\[...\\] delimiters are
    automatically converted to Streamlit-compatible format.
    ...
    # Convert LaTeX delimiters for proper math rendering
    if content:
...
def render_concept_with_navigation(
    ...
    Args:
        ...
        is_loading: Whether navigation is in progress (disables buttons).
    """
    # Module context
    if module_title:
    ...
    with col1:
        if can_go_previous and concept_idx > 0:
            prev_label = "⏳ Loading..." if is_loading else "◀ Previous"
            if st.button(
                prev_label,
                key="prev_concept",
                ...
                if on_previous and not is_loading:
                    on_previous()
        else:
            st.empty()
    ...
    with col3:
        if can_go_next and concept_idx < total_concepts - 1:
            next_label = "⏳ Loading..." if is_loading else "Next ▶"
            if st.button(
                next_label,
                key="next_concept",
                use_container_width=True,
